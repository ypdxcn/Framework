...
int CProcessInterfaceKC::Init(CConfig * pCfg)
{
	...
	if (!m_blGessPktInfoInited)
	{
		...
		string sLogFlag;
		if (0 == pCfg->GetProperty("log", sLogFlag))
		{
			m_nLog = FromString<int>(sLogFlag);
			if (m_nLog <0 || m_nLog > 1)
			{
				m_nLog = 0;
				CRLog(E_ERROR, "参数[IFK.log]不在正常范围(0或1),使用默认值:%d", m_nLog);
			}
		}
...
int CProcessInterfaceKC::SendPacket(CPacket & packet)
{
	try
	{
		if (1 == m_nLog)
		{//打印报文日志
			CIpcPacket pkt = dynamic_cast<CIpcPacket&>(packet);
			unsigned int nLen = 0;
			const char *pcBuf = pkt.Encode(nLen);
			CRLog(E_PROINFO, "%s", pcBuf);
		}
		return CAppProcess::SendPacket(packet);
	}
	catch(std::bad_cast)
	{
