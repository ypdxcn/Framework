...
public:
	...
	//----------------------------------------------------------------
	//函数功能：插入一条记录 add by zww 20140918
	//输入参数：mapRec 记录的映射表（使用前创建map时如果有数据项不是string，可以使用strutils::ToString<>()）转换
	//返 回 值：无
	void AddRecord(map<string,string> mapRec);
	//----------------------------------------------------------------
	//函数功能：插入一条字段集合，解析到m_vFields中 add by zww 20140918
	//输入参数：sFields 表示字段集合,由符号"|"分割，例如"CustId|CustName|Capital"
	//返 回 值：无
	void AddFileds(string & sFileds);
};
