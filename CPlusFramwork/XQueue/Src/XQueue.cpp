...
//
LONG CXQueue::Close(BOOL blSignalQuit)
{
...
int CXQueue::GetBlockInf(size_t& nFree, size_t& nUsed, size_t& nTotal)
{
	if (0 == m_pVariableBlock)
		return -1;

	nFree = m_pVariableBlock->FreeDataBlocks;
	nUsed = m_pVariableBlock->UsedDataBlocks;
	nTotal = m_pVariableBlock->NoDataBlocks;
	return 0;
}
...
LONG CXQueue::WaitForSemWrite(DWORD dwTimeOut)
{
	...
	HANDLE szObj[MAXIMUM_WAIT_OBJECTS] = {0};
	szObj[0] = m_hWriteSem;
	szObj[1] = m_hEventQuit;
	DWORD dwCount = 2;

	DWORD dwWaitResult = WaitForMultipleObjects(dwCount, szObj, FALSE, dwTimeOut);
	if (dwWaitResult < WAIT_OBJECT_0 + dwCount)
	{
		if (WAIT_OBJECT_0 + 1 == dwWaitResult)
		{
			dwWaitResult = XQUEUE_ERROR_QUIT_CMD;
		}
		else
		{
