...
private:
	...
	public:
		...
		unsigned int QueueLen(){return m_deqMsg.size();}
	private:
		int ThreadEntry()
		{
			try
...
public:
	...
	int Enque(MSG& oMsg);
	unsigned int QueueLen() {return m_oWriter.QueueLen();}
	int	GetBlockInf(size_t& nFree, size_t& nUsed, size_t& nTotal) {return m_Queue.GetBlockInf(nFree, nUsed, nTotal);}
private:
	int				m_nMode;				//0读 1写 
...
template<typename MSG>
int CXQueueIo<MSG>::GiveSignal2QuitLoop()
{
	//
	CRLog(E_APPINFO,"m_Queue.Close() GiveSignal2QuitLoop To IoThread");
	m_Queue.Close(TRUE);
	return 0;
}
