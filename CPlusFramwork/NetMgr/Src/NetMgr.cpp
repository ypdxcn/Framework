...
int CNetMgr::UnRegOidCond(CNmoModule* pNmo, const string& sOidIns)
{
	...
	if (itmSrc2Cond != m_mapSrc2Cond.end())
	{
		for (vector<CCondition*>::iterator itvCond = (*itmSrc2Cond).second.begin();itvCond != (*itmSrc2Cond).second.end();++itvCond)
		{
			for (map<string,CCondition*>::iterator itmCond = m_mapCond.begin(); itmCond != m_mapCond.end(); )
			{
...
{
	CMutexGuard oGuard(m_MutexSubscrib);
...
int CNetMgr::OnAlmConfirmTimeout()
{
	...
	if (!m_oThisDay.IsToday())
	{
		m_vNotify.clear();
		m_oThisDay.Day2Today();
	}
	return 0;
}
...
int CNetMgr::QueryEvtSimple(vector<CEventSimple>& vEvtSimple) const 
{
	...
	{
		int nCount = 0;
		for (vector<CEventSimple>::const_iterator it = m_vEvtSimpleTbl.begin(); it != m_vEvtSimpleTbl.end(); ++it)
		{
			if (50 < nCount)
				break;

			vEvtSimple.push_back(*it);
			nCount++;
		}
		return 0;
	}
...
int CNetMgr::QueryEvtTrack(vector<CEventTrack>& vEvtTrack) const 
{
	...
	{
		int nCount = 0;
		for (vector<CEventTrack>::const_iterator it = m_vEvtTrackTbl.begin(); it != m_vEvtTrackTbl.end(); ++it)
		{
			if (50 < nCount)
				break;

			vEvtTrack.push_back(*it);
			nCount++;
		}
		return 0;
	}
...
int CNetMgr::QueryAlarm(vector<CAlarm>& vAlm) const 
{
	...
	{
		int nCount = 0;
		for (vector<CAlarm>::const_iterator it = m_vAlarmTbl.begin(); it != m_vAlarmTbl.end(); ++it)
		{
			if (100 < nCount)
				break;
...
{
	m_MutexSubscrib.Lock();
	for (vector<CNmNotifyIf*>::iterator itvSubscrib = m_vNotify.begin(); itvSubscrib != m_vNotify.end(); )
	{
		if (*itvSubscrib == p)
		{
			itvSubscrib = m_vNotify.erase(itvSubscrib);
			break;
		}
		else
		{
