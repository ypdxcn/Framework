...
protected:
	...
	//取通信出流量数据
	void GetOutFlowStatics(unsigned int & uiOutTotalStat,unsigned int & uiOutAveBytes, unsigned int& uiOutBufBytes, unsigned int& uiOutBufPkts, unsigned int& uiOutBufBytesTotal, unsigned int& uiOutBufPktsTotal, unsigned int& uiOutBufCount);

	int CreateTimer(unsigned long& ulTmSpan);
	void DestroyTimer();
...
private:
	...
	unsigned int	m_uiOutBufBytes;
	unsigned int	m_uiOutBufPkts;
	unsigned int	m_uiOutBufBytesTotal;
	unsigned int	m_uiOutBufPktsTotal;
	unsigned int	m_uiOutBufCount;
	unsigned int	m_uiOutTotal;
	unsigned int	m_uiOutNowStat;
	unsigned int	m_uiOutAveBytes;
	CGessTime		m_oBeginStatTm;
	CGessMutex		m_csProcess;
};
...
public:
	...
	void GetFlowStatics(unsigned int& uiInTotalStat,unsigned int& uiInAveBytes,unsigned int& uiOutTotalStat,unsigned int& uiOutAveBytes, unsigned int& uiOutBufBytes, unsigned int& uiOutBufPkts, unsigned int& uiOutBufBytesTotal, unsigned int& uiOutBufPktsTotal, unsigned int& uiOutBufCount)
	{
		...
		GetOutFlowStatics(uiOutTotalStat,uiOutAveBytes,uiOutBufBytes,uiOutBufPkts,uiOutBufBytesTotal,uiOutBufPktsTotal,uiOutBufCount);
	}
};
...
#define MAX_TCP_SHORT_PACKET_SIZE			1024
//封装TCP短连接协议流程
...
public:
	...
	virtual int SendPacket(CPacket &sndPacket,CPacket &rcvPacket,unsigned int uiTimeout)
	{
		...
		std::list<DATA_BUF*> lstBuf;
		DATA_BUF * pBufNow = new DATA_BUF;
		...
		do
		{
			...
			int nRecv = m_pComm->RecvData(pBuf,nLenLeft,uiTimeoutRcv);
			if (-1 == nRecv)
			{
				break;
			}
